...
from .layers import GeneratorBlockProgressive, GeneratorFirstBlockProgressive, DiscriminatorBlockProgressive, \
    ToRGB, FromRGB, DiscriminatorFinalBlockProgressive, MappingScale

class MyModel(Model):
    def __init__(self, model_string, *args, **kwargs):
    ...
    def store(self, dir):
        for layer in self.layers:
            ...
            for i, weight in enumerate(weights):
                np.save(os.path.join(dir, f'{self.model_string}_{layer.name}_{i}.npy'), weight)

    def restore(self, dir):
        for layer in self.layers:
            weights = []
            layer_file_prefix = f'{self.model_string}_{layer.name}'
...
def generator(resolution, channels):
    class Generator(MyModel):
        def __init__(self, *args, **kwargs):
            super().__init__(model_string='generator', *args, **kwargs)
...
def sndcgan(model_dir, resolution, channels, input_dims, g_lr, d_lr):
    ...
    nn = ProgGAN(model_dir,
                 generator=generator(resolution, channels),
                 discriminator=discriminator(resolution),
                 generator_optimizer=generator_optimizer,
                 discriminator_optimizer=discriminator_optimizer)
    return nn

def generator_mnist(resolution, noise_dims, map_depth):
    class Generator(MyModel):
        def __init__(self, *args, **kwargs):
            super().__init__(model_string='generator', *args, **kwargs)
            self.resolution = resolution
            self.mapper = MappingScale(noise_dims, map_depth)
            self.first_layer = GeneratorFirstBlockProgressive(2, name='input_layer')
            ...
            for res in range(max(2, resolution - 1), resolution + 1):
                setattr(self, f'out_{res}', ToRGB(1, name=f'to_rgb_{res}'))
...
def discriminator_mnist(resolution):
    class Discriminator(MyModel):
        def __init__(self, *args, **kwargs):
            super().__init__(model_string='discriminator', *args, **kwargs)
            self.resolution = resolution
...
def gan_mnist(model_dir, resolution, noise_dims, map_depth, g_lr, d_lr):
    ...
    nn = MnistGAN(model_dir,
                 generator=generator_mnist(resolution, noise_dims, map_depth),
                 discriminator=discriminator_mnist(resolution),
                 generator_optimizer=generator_optimizer,
                 discriminator_optimizer=discriminator_optimizer)
    return nn