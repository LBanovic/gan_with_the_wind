...
class MnistLoader(Loader):
    ...
    def load(self, batch_size, noise_dims):
        ...
        train_dataset = tf.data.Dataset.from_tensor_slices(train_images).shuffle(10000).batch(batch_size, drop_remainder=True).prefetch(10000)
        ...
        test_dataset = tf.data.Dataset.from_tensor_slices(test_images).shuffle(10000).batch(batch_size,drop_remainder=True).prefetch(10000)
